      ...
      <section className="relative w-full bg-[#1a3a28] py-16 px-6">
        ...
        <div className="relative z-10 mx-auto max-w-6xl flex flex-col items-center gap-10">
          ...
          <div className="reveal-up flex flex-col items-center text-center">
            ...
            <h2 className="font-['Playfair_Display',serif] text-2xl md:text-4xl font-bold leading-snug text-white">
              We are What you Need
            </h2>
            <p className="text-gray-300 text-sm md:text-base max-w-xl leading-relaxed mt-2">
          ...
          <div className="w-full grid grid-cols-2 lg:grid-cols-4 gap-6 md:gap-10">
            {features.map((feature, index) => (
              <div
                key={feature.title}
                className="reveal-up flex flex-col items-start gap-3"
                style={{ transitionDelay: `${index * 120}ms` }}
              >
                <div className="reveal-fade" style={{ transitionDelay: `${index * 120 + 200}ms` }}>
                  {feature.icon}
                </div>
                <h3 className="text-white text-sm md:text-base font-semibold">{feature.title}</h3>
                <div className="flex items-center w-full">
                  <button className="flex-shrink-0 w-6 h-6 md:w-7 md:h-7 rounded-full border-2 border-[#4caf72] flex items-center justify-center hover:bg-[#4caf72]/20 transition-colors duration-300">
                    <FaChevronRight size={8} className="text-[#4caf72]" />
                  </button>
